...
public class MeetingSchedulerService {
  ...
  public MeetingSchedulerService(
      MeetingSchedulerRepository repository, RoomSelectionStrategy roomSelectionStrategy) {
  ...
  public synchronized Meeting scheduleMeeting(
    ...
    if (selectedRoom == null) {
      throw new MeetingSchedulerException(
          "No available room found for capacity " + requiredCapacity + " during " + timeSlot);
    }

    String meetingId = "MTG-" + meetingCounter.incrementAndGet();
    Meeting meeting =
        new Meeting(meetingId, subject, organizer, participants, selectedRoom, timeSlot);
