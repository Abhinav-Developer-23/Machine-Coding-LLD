...
import org.example.strategies.CreditCardPaymentStrategy;
import org.example.strategies.WeekdayPricingStrategy;
...
public class MovieBookingDemo {
    public static void main(String[] args) {
        ...
        // 4. Add Cinemas
        service.addCinema("cinema1", "AMC Times Square", nyc.getId(), List.of(screen1));

        // 5. Add Shows
        Show matrixShow =
                service.addShow(
                        "show1", matrix, screen1, LocalDateTime.now().plusHours(2), new WeekdayPricingStrategy());
        Show avengersShow =
                service.addShow(
                        "show2",
                        avengers,
        ...
        // Simulate movie release notification
        ...
        List<Seat> availableSeats =
                selectedShow.getScreen().getSeats().stream()
                        .filter(seat -> seat.getStatus() == SeatStatus.AVAILABLE)
                        .collect(Collectors.toList());
        System.out.printf(
                "Available seats for '%s' at %s: %s%n",
        ...
        System.out.println(
                "Alice selects seats: "
                        + desiredSeats.stream().map(Seat::getId).collect(Collectors.toList()));
        ...
        if (bookingOpt.isPresent()) {
            ...
            System.out.println(
                    "Seats: " + booking.getSeats().stream().map(Seat::getId).collect(Collectors.toList()));
        ...
        desiredSeats.forEach(
                seat -> System.out.printf("Seat %s status: %s%n", seat.getId(), seat.getStatus()));
