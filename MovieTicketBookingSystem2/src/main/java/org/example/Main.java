...
import org.example.strategies.DebitCardStrategy;
import org.example.strategies.ISeatLockProvider;
import org.example.strategies.SeatLockProvider;
  ...
  public static void main(String[] args) {
    try {
      ...
      ShowController showController =
          new ShowController(seatAvailabilityService, showService, theatreService, movieService);
      BookingController bookingController =
          new BookingController(showService, bookingService, theatreService);
      ...
      for (int row = 1; row <= 5; row++) {
        ...
        for (int seatNum = 1; seatNum <= 10; seatNum++) {
          ...
          System.out.println(
              "Created seat at row " + row + " with ID: " + seatId + " and category: " + category);
        }
      }
      ...
      System.out.println(
          "User created: " + user.getUserName() + " with email: " + user.getUserEmail());
          ...
          new Thread(
              () -> {
                try {
                  ...
                  String bookingIdT1 =
                      bookingController.createBooking(user, showId, Arrays.asList(5, 6, 7));
                  System.out.println(
          ...
          new Thread(
              () -> {
                try {
                  ...
                  String bookingIdT2 =
                      bookingController.createBooking(user2, showId, Arrays.asList(7, 8, 9));
                  System.out.println(
      ...
      System.out.println(
          "\nFinal available seats after concurrent booking attempts: "
              + showController.getAvailableSeats(showId));
    } catch (Exception e) {
