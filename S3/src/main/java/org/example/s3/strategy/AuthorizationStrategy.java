 ...
 * Implements fallback and override logic where file-level ACL configurations override bucket-level
 * ones.
 */
public class AuthorizationStrategy {

  /**
   * Evaluates if a user can access a bucket. Access is allowed if the user is the bucket owner or
   * has explicit bucket-level ACL permission.
   *
  ...
  /**
   * Evaluates if a user can access a specific file/object. Access is allowed if: 1. The user is the
   * bucket owner or the file owner. 2. The file has an explicit ACL entry for this user, in which
   * case the file ACL determines access. 3. The file has no explicit ACL entry for this user, in
   * which case access falls back to bucket-level ACL.
   *
   * @param userId the ID of the user requesting access
