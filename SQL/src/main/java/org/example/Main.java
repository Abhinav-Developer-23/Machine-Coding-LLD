    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<Column> columns = Arrays.asList(
                    new Column.Builder("id", ColumnType.INT)
                            .required()
                            .addConstraint(new IntMinValueConstraint()) // min 1024
                            .build(),
                    new Column.Builder("name", ColumnType.STRING)
                            .required()
                            .addConstraint(new StringMaxLengthConstraint()) // max 20 chars
                            .build(),
                    new Column.Builder("department", ColumnType.STRING)
                            .addConstraint(new StringMaxLengthConstraint())
                            .build(),
                    new Column.Builder("salary", ColumnType.INT)
                            .addConstraint(new IntMinValueConstraint(5000)) // custom min
                            .build()
            );
            ...
            // 7. Delete table
            db.createTable("temp_table", Arrays.asList(
                    new Column.Builder("col1", ColumnType.STRING).build()
            ));
